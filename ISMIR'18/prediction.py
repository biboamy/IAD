#!/usr/bin/python
# -*- coding: UTF-8 -*-
import sys,os,torch, librosa, datetime, pickle
...
def main(argv):
    ...
    save_dic = torch.load('./data/model/'+model_choose, map_location={'cuda:0': 'cpu'}, pickle_module=pickle, **pickle_load_args) 
    ...
    #predict configure
    v_kwargs = {'batch_size': 8, 'num_workers': 2}
    ...
    for idx,_input in enumerate(loader):
        ...
        all_pred[ds: ds + len(data)] = torch.sigmoid(pred.squeeze()).data.cpu().numpy()
        ds += len(data)
